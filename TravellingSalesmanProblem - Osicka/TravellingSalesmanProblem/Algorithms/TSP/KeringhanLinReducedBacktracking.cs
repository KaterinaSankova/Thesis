        ...
        private bool ImprovePath()
        {
            ...
            foreach (var node1 in graph.nodes.OrderBy(x => rand.Next()).ToList())
            {
                ...
                foreach (var node2 in FindNode2(node1))
                {
                    ...
                    foreach (var nextPair in nextPairValues)
                    {
                        if (nextPair == (null, null, null, 0))
                            break;
                        ...
                        Edge addedEdge1 = new(node2, nextPair.Node3);
                        ...
                        ImprovePathFromBrokenEdge2(nextPair.Node3, nextPair.Node4, currentPath);
                        if (improvement > 0)
                        {
                        ...
                        if (nextPair.AlternativeNode4 != node1)
                            ImprovePathFromAlternativeBrokenEdge2(node2, nextPair.Node3, nextPair.AlternativeNode4, currentPath);
                        if (improvement > 0)
                        {
                            //UpdateLocalOptimum(shortestPath);
                            //AddCheckOutNodes(node1, path);
