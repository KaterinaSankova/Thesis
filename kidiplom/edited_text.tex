...
O tomto problému je známo, že je \textbf{NP}\textendash úplný. Navíc se dá snadno převést na TSP. To uděláme tak, že pro každou hranu $e$ úplného grafu s vrcholy původní instance nastavíme ohodnocení následovně:
$$c_e = \begin{cases}
1  & \text{for } e \in E\\
|V|*2^{|V|} & \text{for } e \not\in E\\
\end{cases}$$
Pokud bude $OPT_{TSP} = |V|$, pak v původním grafu existovala hamiltonovská kružnice, v opačném případě bude $OPT_{TSP} \ge |V|*2^{|V|}$. Uvědomme si, že hrany s cenou $|V|*2^{|V|}$ budou vybrány pouze v případě, kdy nelze sestavit hamiltonovskou kružnici z hran původního grafu.

Díky takovému ohodnocení existuje exponenciálně velká mezera mezi pozitivními a negativními instancemi HC. Pro pozitivní instance by tedy algoritmus našel v nejhorším případě 
